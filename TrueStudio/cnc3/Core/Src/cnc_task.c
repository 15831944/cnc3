...
uint8_t cnc_ena;
static BOOL enc_xy_ena = FALSE;

void cnc_useEncXY(BOOL ena) {
	if (state == ST_IDLE)
		enc_xy_ena = ena;
}
BOOL cnc_encXYUsed() { return enc_xy_ena; }
		...
		if (cnc_rb())
			...
			if (enc_xy_ena) {
				...
				if (enc_changed) {
					...
					fpoint_t mtr_mm;
					mtr_mm.x = cnc_enc2mmX(enc_x) + cnc_steps2mmX(dx_enc);
					mtr_mm.y = cnc_enc2mmX(enc_y) + cnc_steps2mmX(dy_enc);

#ifdef PRINT
					printf("ENC\nOld (%d %d) Enc (%d %d) dXY (%d %d) ID %d\n", (int)mtr_pt.x, (int)mtr_pt.y, (int)enc_x, (int)enc_y, (int)dx_enc, (int)dy_enc, (int)step_id);
#endif
					mtr_pt.x = (int32_t)round(mtr_mm.x);
					mtr_pt.y = (int32_t)round(mtr_mm.y);
