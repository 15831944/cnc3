...
UVSizingWidget::UVSizingWidget(ProgramParam& par, QWidget *parent) : QWidget(parent), par(par) {
    labelMeasuring = new QLabel(R"(<h1><br>)" + tr("Measuring") + R"(</h1>)");
    groupMeasuring = new QGroupBox;
    labelWireD = new QLabel(tr("Wire diameter") + ":");
    labelUMax = new QLabel(tr("UV-axes maximum value") + " (UV_MAX):");
    ...
    labelLine = new QLabel(tr("Cutline length") + ":");

    btnGen = new QPushButton(tr("Generate"));

    labelRecalc = new QLabel(R"(<h2><br>)" + tr("Recalculate") + R"(</h2>)");
    groupRecalc = new QGroupBox;
    labelDTop = new QLabel(tr("Measuring top diameter") + ":");
    labelDBot = new QLabel(tr("Measuring bot diameter") + ":");
    ...
    groupAxis = new QGroupBox(tr("Axis and direction"));
    ...
    gridMeasuring->addWidget(labelLine, 5, 0, Qt::AlignRight);
    gridMeasuring->addWidget(fnumLine, 5, 1);
    gridMeasuring->addWidget(groupAxis, 6, 0, 1, 2);
    gridMeasuring->addWidget(btnGen, 7, 1);
    ...
    gridRecalc->addWidget(labelDTop, 0, 0, Qt::AlignRight);
    gridRecalc->addWidget(fnumDTop, 0, 1, Qt::AlignLeft);
    gridRecalc->addWidget(labelDBot, 1, 0, Qt::AlignRight);
    gridRecalc->addWidget(fnumDBot, 1, 1, Qt::AlignLeft);
    gridRecalc->addWidget(btnRecalc, 2, 1);
...
//    }
    ...
    widgets = {
        labelWireD, labelUMax, labelL, labelH, labelT, labelLine, labelDTop, labelDBot, labelResL, labelResH,
