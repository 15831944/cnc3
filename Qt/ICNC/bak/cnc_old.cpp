...
CncComThread* Cnc::m_com = nullptr;

Cnc::Cnc() : m_msg(nullptr) {
    m_fpga.bind(&m_com);
}
Cnc::~Cnc() {
    close();
...
void Cnc::setReportWriter(Reporter msg) {
    this->m_msg = msg;
}

bool Cnc::open(string /*portName*/) {
    if (!m_com) {
        m_com = new CncComThread(m_msg);
        if (!m_com)
            return false;
    ...
    if (nx || ny || nu || nv) {
        try {            
            ...
            if (isOpen()) {
                ...
                int32_t Tx = nx ? aux_func::double_to_int32(ts / fabs(nx)) : 0;
                int32_t Ty = ny ? aux_func::double_to_int32(ts / fabs(ny)) : 0;
                int32_t Tu = nu ? aux_func::double_to_int32(ts / fabs(nu)) : 0;
                int32_t Tv = nv ? aux_func::double_to_int32(ts / fabs(nv)) : 0;
