...
void GCode::push_back(const GeneratorMode& genMode) {
    frames.push_back(GFrame::M102(genMode.pulse_width, genMode.pulse_ratio));
    append(GFrame::M103(genMode.current_level));
    append(GFrame::M104(genMode.voltage_level));
    append(GFrame::M105(genMode.drum_velocity));
}
...
bool GCode::generate(const cut_t& cut, const deque<ContourPair>& contours) {
    frames.clear();
...
//                        frames.push_back(GFrame::M0());
                    ...
                    M105.value = ent->hasDrumVel() ? ent->drumVel() : 7;
                    M105.init = true;

                    frames.push_back(GFrame::M105(M105.value));
#else
                    push_back(pair.generatorMode());
#endif

//                    frames.push_back(GFrame::M82());
//                    frames.push_back(GFrame::M84());
...
//                    frames.push_back(GFrame::M0()); // stop
                    ...
                    else if (cut.tab_pause && !tab && pair.type() == CONTOUR_TYPE::CONTOUR_TAB) {
                        tab = true;
                ...
                while (bot || top) {
                    if (bot) {
                        if (bot->type() == ENTITY_TYPE::LINE) {
                            if (bot->hasDrumVel()) {
                                if (!M105.init || M105.value != bot->drumVel()) {
                                    M105.value = bot->drumVel();
                                    frames.push_back(GFrame::M105(M105.value));
                                }
                            }
                    ...
                    if (top) {
                        if (bot->type() == ENTITY_TYPE::LINE) {
                            if (G_reg != 1)
                                frames.push_back(GFrame::G1());
                            append(GEntity('U', bot->point_1().x), GEntity('V', bot->point_1().y));
                        }
                        else if (bot->type() == ENTITY_TYPE::ARC) {
                            if (bot->CCW()) {
                                if (G_reg != 3)
                    ...
                    if (cut.seg_pause && (pair.type() == CONTOUR_TYPE::CONTOUR_PASS || pair.type() == CONTOUR_TYPE::CONTOUR_TAB || (pair.type() == CONTOUR_TYPE::CONTOUR_CUTLINE && i == 0)))
...
bool GCode::generate(double d_top, double d_bot, double L, double H, double T, double cutline, AXIS axis) {
    frames.clear();
    frames.push_back(GFrame::delimiter());
    frames.push_back(GFrame::M106(L, H));
    frames.push_back(GFrame::M107(T));
...
bool GCode::CompareFrames(const std::list<string>& ref, const std::list<string>& cmp, string& error_text) {
    ...
    for (; it1 != ref.end() && it2 != cmp.end(); ++it1, ++it2, i++) {
        ...
        if (s1.compare(s2) != 0) {
            ...
            error_text += "Reference frame number is " + s1 + "\n";
            error_text += "Compared frame number is " + s2;
            return false;
        }
    }
    ...
    error_text = "Frames count error. Reference frames number is " + std::to_string(ref.size()) + ", compared frames number is " + std::to_string(cmp.size());
    return false;
}

string GCode::toText(list<string> &gframes) {
    string txt;
