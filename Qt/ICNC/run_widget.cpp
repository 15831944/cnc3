...
void RunWidget::createState() {
    ...
    labelEncX = new QLabel(R"(<font color=black>)" + tr("EncX") + R"(</font>)");
    labelEncY = new QLabel(R"(<font color=black>)" + tr("EncY") + R"(</font>)");
    ...
    gridState->addWidget(labelEncX, 9, 0);
    gridState->addWidget(posLabels[6], 9, 1, Qt::AlignRight);

    gridState->addWidget(labelEncY, 10, 0);
    gridState->addWidget(posLabels[7], 10, 1, Qt::AlignRight);
#endif
...
//        QString s2 = QString::asprintf("%c%07d", sign ? '-' : ' ', abs_value);
        ...
        posLabels[6 + axis_num]->setText(R"(<font color=black>)" + s + R"(</font>)");
    }
}
