...
bool Cnc::testRegs() {
    ...
    uint32_t test_reg = m_com.read32(ADDR::TEST_REG);
    ...
    test_reg = m_com.read32(ADDR::TEST_REG);
...
bool Cnc::testProgArray() {
    ...
    uint32_t rdaddr = m_com.read32(ADDR::PA_RDADDR);
    uint32_t wraddr = m_com.read32(ADDR::PA_WRADDR);
    uint32_t size = m_com.read32(ADDR::PA_SIZE);
    ...
    vector<uint8_t> rddata = m_com.read(ADDR::PA, size);
