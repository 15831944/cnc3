...
public:
    ...
    std::string toString() {
        return auxItems::string_format("%d.%d.%d.%d%c%s",
            static_cast<int>(fields.fac_ver), static_cast<int>(fields.fac_rev), static_cast<int>(fields.ver), static_cast<int>(fields.rev),
            fields.type == 0 ? ' ' : fields.type == 1 ? 'a' : fields.type == 2 ? 'b' : 'u', fields.stone ? " (stone)" : ""
        );
    }
};
