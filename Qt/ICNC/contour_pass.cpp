...
deque<ContourPair>& ContourPass::generate() {
    ...
    for (const ContourPair& pair: m_base.contours()) {
        if (!pair.empty())
            switch (pair.type()) {
            case CONTOUR_TYPE::CONTOUR_MAIN:
            {
                deque<ContourPair> pass;
                deque<ContourPair> tab;
                deque<ContourPair> overlap;
                ContourPair out;
                ...
                ContourPair aux_pair(CONTOUR_TYPE::CONTOUR_PASS, pair);
                ...
                double overlap_value = aux_pair.cut().overlap;

                if (tab_seg != 0 || tab_value < M_TAB_MIN || tab_value > (aux_pair.lengthBot() - M_TAB_MIN))
                    tab_value = 0;

                if (overlap_value < M_OVERLAP_MIN || overlap_value > (aux_pair.lengthBot() - tab_value) / 2)
                    overlap_value = 0;

                {
                    ContourPair ctr(CONTOUR_TYPE::CONTOUR_PASS, aux_pair);
                    ...
                    ctr.setType(CONTOUR_TYPE::CONTOUR_TAB);
                    dir = !dir;
                ...
                {
                    ContourPair ctr(CONTOUR_TYPE::CONTOUR_OVERLAP, aux_pair);
                    size_t bot_idx = 0, top_idx = 0;

                    ctr.split(overlap_value, bot_idx, top_idx);

                    ctr.offset(side, pair.cut().getOverlapOffset().offset);
                    ctr.choiceMode(pair.cut().getOverlapOffset().mode_id);
                    ctr.remove_from(bot_idx, top_idx);

                    ContourPair clone(ctr);
                    clone.reverse();

                    overlap.push_back(move(ctr));
                    overlap.push_back(move(clone));
                }
...
//                }
                ...
                printDebug(pass, tab, overlap, out);
                deque<ContourPair> ctr;

                if (aux_pair.isLoop() && !aux_pair.hasOut())
                    ctr = joinLoop(pass, tab, overlap);
                else
                    ctr = join(pass, tab, overlap, out);

                move_back(ctr);
            }
                break;
            case CONTOUR_TYPE::CONTOUR_CUTLINE:
                printDebug(pair);
                copy_back(pair);
                break;
            default:
                ...
                if (overlap_value < M_OVERLAP_MIN || overlap_value > aux_pair.lengthBot() / 2)
                    overlap_value = 0;
...
void ContourPass::connect(const ContourPair& prev, ContourPair& next) {
    if (!prev.emptyBot() && !next.emptyBot() && prev.bot()->back() && next.bot()->front()) {
    ...
    if (!prev.emptyTop() && !next.emptyTop() && prev.top()->back() && next.top()->front()) {
...
deque<ContourPair> ContourPass::join(deque<ContourPair> &pass, deque<ContourPair> &tab, deque<ContourPair> &overlap) {
...
deque<ContourPair> ContourPass::join(deque<ContourPair>& pass, deque<ContourPair>& tab, deque<ContourPair>& overlap, ContourPair& out) {
    deque<ContourPair> contours = join(pass, tab, overlap);
    join(contours, out);
    return contours;
}

deque<ContourPair> ContourPass::joinLoop(deque<ContourPair> &pass, deque<ContourPair> &tab, deque<ContourPair> &overlap) {
    deque<ContourPair> contours = join(pass, tab, overlap);

    ContourPair out(CONTOUR_TYPE::CONTOUR_OUT);
    out.setGeneratorModes(contours.back());
    const ContourPair& last = contours.back();
    const ContourPair& first = contours.front();

    if (!first.emptyBot() && !last.emptyBot()) {
        const fpoint_valid_t A = last.bot()->last_point();
        const fpoint_valid_t B = first.bot()->first_point();

        if (A.valid && B.valid && A != B) out.bot()->push_back( DxfLine(A, B) );
    }

    if (!first.emptyTop() && !last.emptyTop()) {
        const fpoint_valid_t A = last.top()->last_point();
        const fpoint_valid_t B = first.top()->first_point();

        if (A.valid && B.valid && A != B) out.top()->push_back( DxfLine(A, B) );
    }
...
void ContourPass::copy_back(const ContourPair& pair) {
    ...
    else if (pair.type() == CONTOUR_TYPE::CONTOUR_CUTLINE && !pair.empty()) {
...
void ContourPass::move_back(deque<ContourPair>& ctr) {
    if (!ctr.empty()) {
        if (m_contours.empty())
            m_contours = ctr;
        else if (m_contours.back().type() == CONTOUR_TYPE::CONTOUR_CUTLINE && !ctr.front().empty()) {
...
void ContourPass::printDebug(const deque<ContourPair>& pass, const deque<ContourPair>& tab, const deque<ContourPair>& overlap, const ContourPair& out) {
    printDebug(pass);
    printDebug(tab);
    printDebug(overlap);
    printDebug(out);
