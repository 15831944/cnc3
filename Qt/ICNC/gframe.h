...
public:
    ...
    // pump enable
    static GFrame M40() { return GFrame(GEntity('M', uint8_t(40)), "Pump enable"); }

    // pump disable
    static GFrame M41() { return GFrame(GEntity('M', uint8_t(41)), "Pump disable"); }

    // drum enable
    static GFrame M82() { return GFrame(GEntity('M', uint8_t(82)), "Drum enable"); }

    // drum disable
    static GFrame M83() { return GFrame(GEntity('M', uint8_t(83)), "Drum disable"); }

    // voltage enable
    static GFrame M84() { return GFrame(GEntity('M', uint8_t(84)), "Voltage enable"); }

    // voltage disable
    static GFrame M85() { return GFrame(GEntity('M', uint8_t(85)), "Voltage disable"); }
...
//    static GFrame M100() { return GFrame(GEntity('M', uint8_t(100)), "Apply"); }
    ...
    // pulse
    static GFrame M102(float width, float ratio) { return GFrame(GEntity('M', uint8_t(102)), GEntity('P', double(width)), GEntity('Q', double(ratio))); }

    // current
    static GFrame M103(float value) { return GFrame(GEntity('M', uint8_t(103)), GEntity('P', double(value))); }

    // voltage level
    static GFrame M104(float value) { return GFrame(GEntity('M', uint8_t(104)), GEntity('P', double(value))); }

    // drum velocity
    static GFrame M105(float value) { return GFrame(GEntity('M', uint8_t(105)), GEntity('P', double(value))); }

    // Second contour settings
    static GFrame M106(float L_rollers, float H_bot) {
        return GFrame(GEntity('M', uint8_t(106)), GEntity('P', double(L_rollers)), GEntity('Q', double(H_bot)));
    }

    // Workpeace thickness
    static GFrame M107(float Thickness) {
        return GFrame(GEntity('M', uint8_t(107)), GEntity('P', double(Thickness)));
    }
